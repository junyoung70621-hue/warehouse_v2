...
@st.fragment
def _render_interactive(df_all, raw_data, categories):
    # session_state에서 현재 컨텍스트 재취득
    selected_center = st.session_state.get('sidebar_center', '자재센터')
    user            = st.session_state.user
    user_id         = user['id']
    user_name       = user['name']
    user_role       = user['role']
    ...
    CAN_TRANSFER       = can_request_transfer(user, selected_center)
    SHOW_STOCK_BUTTONS = CAN_STOCK and IS_MAIN_HUB
    CAN_USAGE_UPLOAD   = (not IS_MAIN_HUB and (user_role == 'admin' or
                          (user_role == 'manager' and _get_center(user) == selected_center)))
    CAN_MATERIAL_REQUEST = (not IS_MAIN_HUB and user_role not in ('guest', 'materials'))
...
_render_interactive(df_all, raw_data, categories)
