...
if False:
    ...
    # ── 새 배정 ──────────────────────────────────────────────────────────────
    if _can_write:
        with st.expander("➕ 새 배정", expanded=False):
            center_users = fetch_center_users(sel_center)
            if not center_users:
                st.info("해당 센터에 승인된 직원이 없습니다.")
            else:
                if _can_manage or user_role == "manager":
                    user_opts    = {u.get("name") or u["username"]: u for u in center_users}
                    sel_emp_name = st.selectbox("배정 직원", list(user_opts.keys()), key="btt_new_emp")
                    tgt_u        = user_opts[sel_emp_name]
                    tgt_id, tgt_name = tgt_u["id"], tgt_u.get("name") or tgt_u["username"]
                else:
                    tgt_id, tgt_name = user_id, user_name
                    st.markdown(f"배정 직원: **{tgt_name}**")

                with st.spinner("배정 가능한 단말기 조회 중..."):
                    available = get_available_terminals(sel_center)

                if not available:
                    st.info("배정 가능한 단말기가 없습니다. (버스단말기 현황 → 출고 업로드 필요)")
                else:
                    avail_df = pd.DataFrame(available)
                    avail_df.rename(columns={"ih_code": "IH", "device_type": "기종",
                                             "sub_type": "유형", "upload_date": "출고일"}, inplace=True)
                    ...
                    edited_new = st.data_editor(
                        ...
                        use_container_width=True, hide_index=True, key="btt_new_editor",
                    )
                    sel_new = edited_new[edited_new["선택"] == True]
                    if not sel_new.empty:
                        if st.button(f"✅ {tgt_name}에게 배정 ({len(sel_new)}건)",
                                     type="primary", key="btt_new_btn"):
                            items = [{"ih_code": r["IH"], "device_type": r["기종"], "sub_type": r["유형"]}
                                     for _, r in sel_new.iterrows()]
                            if assign_terminals(items, sel_center, tgt_id, tgt_name, user_id):
                                st.success(f"✅ {len(items)}건 배정 완료!")
                                st.rerun()
    ...
    with cf1:
        status_filter = st.radio(
            "상태", ["전체", "양품 보유중", "불량 보유중", "교체 완료", "반납 완료"],
            horizontal=True, key="btt_sf", label_visibility="collapsed",
        )
    with cf2:
        if st.button("새로고침", use_container_width=True, key="btt_refresh"):
            st.rerun()

    _sf_map = {
        "전체": None, "양품 보유중": "holding", "불량 보유중": "defective",
        "교체 완료": "exchanged", "반납 완료": "returned",
    }
    ...
    if not rows:
        ...
        m1.metric("양품 보유중", int((main_df["status"] == "holding").sum()))
        m2.metric("불량 보유중", int((main_df["status"] == "defective").sum()))
        m3.metric("교체 완료",   int((main_df["status"] == "exchanged").sum()))
        m4.metric("반납 완료",   int((main_df["status"] == "returned").sum()))

        disp_df = main_df[["employee_name", "ih_code", "device_type", "sub_type", "배정일시", "상태라벨"]].copy()
        disp_df.columns = ["직원명", "IH", "기종", "유형", "배정일시", "상태"]
        ...
        edited_main = st.data_editor(
            ...
            use_container_width=True, hide_index=True, key="btt_main_editor",
        )

        sel_rows = edited_main[edited_main["선택"] == True]

        # ── 선택 시 액션 버튼 ──────────────────────────────────────────────
        if not sel_rows.empty:
            sel_idx      = sel_rows.index.tolist()
            sel_records  = main_df.iloc[sel_idx]
            sel_ids      = sel_records["id"].tolist()
            holding_sel  = sel_records[sel_records["status"] == "holding"]
            defect_sel   = sel_records[sel_records["status"] == "defective"]
            ...
            # ── 직원 이동 ────────────────────────────────────────────────
            with ac1:
                if st.button("👤 직원 이동", use_container_width=True, key="btt_act_transfer"):
                    st.session_state["btt_action"] = "transfer"

            # ── 불량 교체 (양품 1건 선택 시만) ──────────────────────────
            with ac2:
                swap_ok = len(holding_sel) == 1 and len(sel_ids) == 1
                if st.button("🔄 불량 교체", use_container_width=True, key="btt_act_swap",
                             disabled=not swap_ok,
                             help="양품(양품 보유중) 1건만 선택해야 합니다."):
                    st.session_state["btt_action"] = "swap"

            # ── 반납 처리 ────────────────────────────────────────────────
            with ac3:
                if st.button("↩️ 반납 처리", use_container_width=True, key="btt_act_return"):
                    st.session_state["btt_action"] = "return"

            # ── 배정 취소 ────────────────────────────────────────────────
            with ac4:
                if st.button("🗑️ 배정 취소", use_container_width=True, key="btt_act_cancel"):
                    st.session_state["btt_action"] = "cancel"

            # ── 액션 패널 ────────────────────────────────────────────────
            action = st.session_state.get("btt_action")

            if action == "transfer":
                with st.container(border=True):
                    st.markdown("**직원 이동** — 선택한 단말기를 다른 직원에게 이동합니다.")
                    cus = fetch_center_users(sel_center)
                    if cus:
                        u_opts = {u.get("name") or u["username"]: u for u in cus}
                        t_name = st.selectbox("이동할 직원", list(u_opts.keys()), key="btt_tr_emp")
                        t_u    = u_opts[t_name]
                        bc1, bc2 = st.columns(2)
                        if bc1.button("✅ 이동 확정", type="primary", key="btt_tr_ok"):
                            try:
                                get_supabase().table(TABLE).update({
                                    "employee_id":   t_u["id"],
                                    "employee_name": t_u.get("name") or t_u["username"],
                                    "assigned_at":   _now_kst().isoformat(),
                                    "assigned_by":   user_id,
                                }).in_("id", sel_ids).execute()
                                st.session_state.pop("btt_action", None)
                                st.success(f"✅ {len(sel_ids)}건 → {t_u.get('name') or t_u['username']} 이동 완료!")
                                st.rerun()
                            except Exception as e:
                                st.error(f"이동 실패: {e}")
                        if bc2.button("취소", key="btt_tr_cancel"):
                            st.session_state.pop("btt_action", None)
                            st.rerun()
                ...
                with st.container(border=True):
                    ...
                    def_ih = st.text_input("수거한 불량 IH 번호", key="btt_sw_ih",
                                           placeholder="예: 100456")
                    bc1, bc2 = st.columns(2)
                    if bc1.button("✅ 교체 확정", type="primary", key="btt_sw_ok"):
                        if not def_ih.strip():
                            st.warning("불량 IH 번호를 입력해 주세요.")
                        else:
                            if swap_terminal(rec["id"], def_ih.strip(), sel_center,
                                             rec.get("employee_id"), rec["employee_name"]):
                                st.session_state.pop("btt_action", None)
                                st.success(f"✅ 교체 완료: 양품 {rec['ih_code']} → 불량 {def_ih.strip()} 등록")
                                st.rerun()
                    if bc2.button("취소", key="btt_sw_cancel"):
                        st.session_state.pop("btt_action", None)
                        st.rerun()

            elif action == "return":
                with st.container(border=True):
                    st.markdown(f"**반납 처리** — 선택 {len(sel_ids)}건을 반납 완료로 처리합니다.")
                    bc1, bc2 = st.columns(2)
                    if bc1.button("✅ 반납 확정", type="primary", key="btt_ret_ok"):
                        if return_terminals(sel_ids):
                            st.session_state.pop("btt_action", None)
                            st.success(f"✅ {len(sel_ids)}건 반납 처리 완료!")
                            st.rerun()
                    if bc2.button("취소", key="btt_ret_cancel"):
                        st.session_state.pop("btt_action", None)
                        st.rerun()

            elif action == "cancel":
                with st.container(border=True):
                    st.markdown(f"**배정 취소** — 선택 {len(sel_ids)}건을 삭제합니다.")
                    st.caption("삭제된 양품 단말기는 배정 가능 풀로 돌아갑니다.")
                    bc1, bc2 = st.columns(2)
                    if bc1.button("✅ 삭제 확정", type="secondary", key="btt_can_ok"):
                        if delete_assignments(sel_ids):
                            st.session_state.pop("btt_action", None)
                            st.success(f"✅ {len(sel_ids)}건 삭제 완료!")
                            st.rerun()
                    if bc2.button("취소", key="btt_can_cancel"):
                        st.session_state.pop("btt_action", None)
                        st.rerun()

        # ── 직원별 요약 ───────────────────────────────────────────────────
        holding_df = main_df[main_df["status"].isin(["holding", "defective"])]
        if not holding_df.empty:
            st.divider()
            st.markdown("##### 직원별 보유 현황")
            grp = holding_df.groupby(["employee_name", "status"])["ih_code"].apply(list).reset_index()
            grp["상태"] = grp["status"].map(_STATUS_LABEL)
            grp["IH 목록"] = grp["ih_code"].apply(lambda x: ", ".join(sorted(x)))
            grp["수량"] = grp["ih_code"].apply(len)
            grp.rename(columns={"employee_name": "직원명"}, inplace=True)
            st.dataframe(grp[["직원명", "상태", "수량", "IH 목록"]],
